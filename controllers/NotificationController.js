...
exports.sendNotification = async (req) => {
  try {
    ...
    if (!pushToken) {
      console.log(
        `Notification simulation for user ${userId}: ${title} - ${body}`,
      );
      return {
        message:
          "Notification simulation successful. No push token found for user.",
        simulated: true,
      };
    }
...
exports.sendNotificationRoute = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Notification sent successfully", tickets });
  } catch (error) {
...
exports.sendNotificationToWorker = async (req) => {
  try {
    ...
    if (!pushToken) {
      console.log(
        `Notification simulation for worker ${workerId}: ${title} - ${body}`,
      );
      return {
        message:
          "Notification simulation successful. No push token found for worker.",
        simulated: true,
      };
    }
